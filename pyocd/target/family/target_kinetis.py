...
class Kinetis(CoreSightTarget):
    ...
    def check_mdm_ap_idr(self):
        ...
        self.mdm_ap = self.dp.aps[ap.APv1Address(1)]
        ...
        if self.session.options.get('connect_mode') != 'under-reset':
            ...
            if isLocked:
                ...
                try:
                    ...
                    ap0 = self.aps[ap.APv1Address(0)]
                    if isinstance(ap0, ap.MEM_AP):
                        # Use an exponentially increasing delay time between attempts.
                        # Total is just under 70 ms for 10 attempts.
                        delay_s = 0.001 # 1 ms
                        for attempt in range(ACCESS_TEST_ATTEMPTS):
                            ap0.read32(CortexM.DHCSR)
                            sleep(delay_s)
                            delay_s *= 1.4
                    else:
                        LOG.warning("%s is unexpectedly not a MEM-AP", ap0.short_description)
