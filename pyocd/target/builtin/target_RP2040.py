...
FLASH_ALGO = {
    ...
    'page_buffers' : [0x20001000, 0x20001000],   # Enable double buffering
    'min_program_length' : 0x0,
...
def _parity32byte(value):
    parity = 0
    for i in range(32):
        parity += value & 1
        value >> 1
    return bytearray([parity & 1])

class RP2040(CoreSightTarget):
    ...
    signal is repurposed as a rescue signal. This is not a good design, but it's what we're given.
    """
    ...
    def create_init_sequence(self):
        ...
        seq.insert_before('dp_init', ('select_core0', self._select_core0))
    ...
    def _select_core0(self):
        self.select_dp(self.Targetsel.CORE_0)

    def select_dp(self, targetsel):
        """! @brief Select the DP for core 0."""
        probe = self.session.probe

        # First send a line reset and at least 2 idle cycles (SWDIO/TMS Low).
        swj = SWJSequenceSender(self.session.probe, True)
        swj.line_reset()
        probe.swj_sequence(8,  0x00)
        
        # Send multi-drop SWD selection sequence.
        #
        # On the control bytes (first byte of tuples), bit 7 is 1 for input, bits [5:0] are SWCK count.
        targetsel_data = bytearray(conversion.u32le_list_to_byte_list([targetsel])) + _parity32byte(targetsel)
        seqs = [
            # DP TARGETSEL write
            # output 8 cycles:
            #   - Start = 1
            ...
            #   - Stop = 0
            #   - Park = 1
            # -> LSB first, that's 0b10111101 or 0xbd
            (0x08, b"\xbd"),
